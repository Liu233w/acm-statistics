...
export const getters = {
  ...
  /**
   * Solved problems for each crawler grouped by crawlerName.
   *
   * Problems in virtual_judge that can be separated into other OJs will be separated.
   * E.g. {vjudge: ['poj-1001', 'NO_NAME-1234']} becomes {vjudge: ['NO_NAME-1234'], poj: ['1001']}
   */
  summaryForCrawler(state) {

    const warnings = []

    /**
     * @type {Object.<string, {crawlerTitle: string, usernames: Set<string>, solvedSet: Set<string>, submissions: number}>}
     */
    const summaries = {}
    for (const crawlerName in state.crawlers) {
      const crawler = state.crawlers[crawlerName]
      summaries[crawlerName] = {
        crawlerTitle: crawler.virtual_judge ? crawler.title + '(Not Merged)' : crawler.title,
        usernames: new Set(),
        solvedSet: new Set(),
        submissions: 0,
      }
    ...
    const doneWorkers = _.filter(state.workers,
      worker => worker.username && worker.status === WORKER_STATUS.DONE && !worker.errorMessage)
    for (let worker of doneWorkers) {

      const crawlerName = worker.crawlerName
      const crawler = state.crawlers[crawlerName]

      if (!worker.solvedList) {
        warnings.push(`${crawler.title} does not have solved list, its result will be ignored.`)
        continue
      }

      const summary = summaries[crawlerName]

      summary.usernames.add(worker.username)
      summary.submissions += worker.submissions

      if (crawler.virtual_judge) {

        for (const problem of worker.solvedList) {
          const [problemCrawlerName, problemId] = _.split(problem, '-', 2)
          if (summaries[problemCrawlerName]) {
            const problemCrawlerSummary = summaries[problemCrawlerName]
            problemCrawlerSummary.usernames.add(`[${worker.username} in ${crawler.title}]`)
            problemCrawlerSummary.solvedSet.add(problemId)
          } else {
            summary.solvedSet.add(problem)
          }
        }

        const nameToSubmissions = worker.submissionsByCrawlerName
        if (nameToSubmissions) {
          for (const problemCrawlerName in nameToSubmissions) {
            if (problemCrawlerName in summaries) {
              summaries[problemCrawlerName].submissions += nameToSubmissions[problemCrawlerName]
              summary.submissions -= nameToSubmissions[problemCrawlerName]
            }
          }
        } else {
          warnings.push(`The return of ${crawler.title} did not contain a submissionsByCrawlerName field, so its submissions are not divided into other crawlers.`)
        }

      } else {
        _.forEach(worker.solvedList, a => summary.solvedSet.add(a))
      }
    }
    return {
      summaries,
